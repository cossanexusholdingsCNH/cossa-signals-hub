...
function validateCandles(candles: Candle[]) {
  ...
  for (const c of candles) {
    if (![c.open, c.high, c.low, c.close].every(finite)) throw new Error('Candle contains a non-finite price');
    if (c.open <= 0 || c.high <= 0 || c.low <= 0 || c.close <= 0) throw new Error('Candle prices must be positive');
...
export function atr(candles: Candle[], period = 14): number {
  ...
  for (let i = 1; i < recent.length; i += 1) {
    ...
    trueRanges.push(Math.max(c.high - c.low, Math.abs(c.high - previousClose), Math.abs(c.low - previousClose)));
  }
...
export function macd(values: number[]) {
  ...
  const line = values.map((_, i) => (finite(fast[i]) && finite(slow[i]) ? fast[i] - slow[i] : Number.NaN));
...
export function buildTradePlan(candles: Candle[]): TradePlan {
  ...
  if (ema20 > ema50) { bullScore += 2; reasons.push('EMA20 is above EMA50'); }
  if (ema20 < ema50) { bearScore += 2; reasons.push('EMA20 is below EMA50'); }
  if (m.histogram > 0) { bullScore += 1; reasons.push('MACD histogram is positive'); }
  if (m.histogram < 0) { bearScore += 1; reasons.push('MACD histogram is negative'); }
  if (rsi14 >= 52 && rsi14 <= 70) { bullScore += 1; reasons.push('RSI supports bullish momentum without extreme overbought conditions'); }
  if (rsi14 <= 48 && rsi14 >= 30) { bearScore += 1; reasons.push('RSI supports bearish momentum without extreme oversold conditions'); }
  ...
  const confidence = Math.min(95, Math.max(0, 50 + Math.abs(edge) * 8 - (regime === 'volatile' ? 10 : 0)));
  if (direction === 'wait') {
    return {
      direction, regime, confidence, entry: null, entryZoneLow: null, entryZoneHigh: null,
      stopLoss: null, takeProfit1: null, takeProfit2: null, takeProfit3: null,
      riskRewardRatio: null,
      indicators: { rsi14, ema20, ema50, atr14, macd: m.macd, macdSignal: m.signal, macdHistogram: m.histogram },
      reasons, noTradeReasons: noTradeReasons.length ? noTradeReasons : ['Directional score did not meet minimum threshold'],
    };
  }
