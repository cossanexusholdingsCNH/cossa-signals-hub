...
export async function evaluatePaperExecution(request: PaperExecutionRequest): Promise<PaperExecutionDecision> {
  ...
  const snapshotResult = await supabaseAdmin
    ...
    .limit(1)
    .maybeSingle();
  if (snapshotResult.error || !snapshotResult.data) throw new Error('Missing account snapshot; execution fails closed');
  ...
  const decision = evaluateExecutionRisk({
    ...
    startOfDayEquity: requireFinitePositive(dailyResult.data.start_of_day_equity, 'start-of-day equity'),
    ...
    emergencyStop: Boolean(account.emergency_stop) || Boolean(dailyResult.data.loss_limit_triggered),
