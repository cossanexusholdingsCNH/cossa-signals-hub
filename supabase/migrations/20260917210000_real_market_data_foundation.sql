...
-- Close the three missing FK-index gaps reported by the Supabase advisor.
...
-- Client roles can read provider health/configuration and real market data.
-- Writes are intentionally service-role only: no INSERT/UPDATE/DELETE grants are
-- given to anon/authenticated for ingestion or engine tables.
...
CREATE POLICY "market providers staff read"
  ON public.market_data_providers FOR SELECT TO authenticated
  USING ((select public.is_staff((select auth.uid()))));

CREATE POLICY "provider mappings staff read"
  ON public.instrument_provider_mappings FOR SELECT TO authenticated
  USING ((select public.is_staff((select auth.uid()))));

CREATE POLICY "market ticks staff read"
  ON public.market_ticks FOR SELECT TO authenticated
  USING ((select public.is_staff((select auth.uid()))));

CREATE POLICY "market candles authenticated read"
  ON public.market_candles FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "ingestion runs staff read"
  ON public.market_data_ingestion_runs FOR SELECT TO authenticated
  USING ((select public.is_staff((select auth.uid()))));

CREATE POLICY "engine runs staff read"
  ON public.signal_engine_runs FOR SELECT TO authenticated
  USING ((select public.is_staff((select auth.uid()))));

-- Keep updated_at consistent with the existing application schema.
CREATE TRIGGER market_data_providers_updated
  BEFORE UPDATE ON public.market_data_providers
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER instrument_provider_mappings_updated
  BEFORE UPDATE ON public.instrument_provider_mappings
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

COMMENT ON TABLE public.market_data_providers IS
  'Editable provider registry. Secrets must never be stored here; use server environment variables/secrets.';
COMMENT ON TABLE public.instrument_provider_mappings IS
  'Maps canonical Cossa instruments to provider-specific symbols without hard-coding provider names in application logic.';
COMMENT ON TABLE public.market_ticks IS
  'Raw real-time ticks received from configured providers. Service-role writes only.';
COMMENT ON TABLE public.market_candles IS
  'Normalized OHLC candle history used by deterministic analysis, backtests and signal generation.';
COMMENT ON TABLE public.market_data_ingestion_runs IS
  'Operational audit trail for provider ingestion, latency, rejection and failure diagnostics.';
COMMENT ON TABLE public.signal_engine_runs IS
  'Auditable decision trail for every signal-engine evaluation, including explicit no-trade outcomes.';
