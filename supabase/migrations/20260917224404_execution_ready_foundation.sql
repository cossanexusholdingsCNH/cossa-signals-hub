...
CREATE TABLE public.execution_orders (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CHECK ((execution_mode = 'paper_auto' AND confirmation_required = false) OR (execution_mode <> 'paper_auto' AND confirmation_required = true)),
  CHECK (execution_mode <> 'live_manual' OR status IN ('draft','awaiting_confirmation','approved','submitted','filled','partially_filled','rejected','cancelled','closed','failed'))
);
...
CREATE POLICY "users read own trading accounts" ON public.trading_accounts FOR SELECT TO authenticated USING (user_id = (select auth.uid()) OR (select public.is_staff((select auth.uid()))));
CREATE POLICY "users create own trading accounts" ON public.trading_accounts FOR INSERT TO authenticated WITH CHECK (user_id = (select auth.uid()));
CREATE POLICY "users update own trading accounts" ON public.trading_accounts FOR UPDATE TO authenticated USING (user_id = (select auth.uid())) WITH CHECK (user_id = (select auth.uid()));
CREATE POLICY "users read own execution orders" ON public.execution_orders FOR SELECT TO authenticated USING (user_id = (select auth.uid()) OR (select public.is_staff((select auth.uid()))));
CREATE POLICY "users read own execution events" ON public.execution_events FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.execution_orders eo WHERE eo.id = order_id AND (eo.user_id = (select auth.uid()) OR (select public.is_staff((select auth.uid()))))));
CREATE POLICY "users read own execution risk checks" ON public.execution_risk_checks FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.execution_orders eo WHERE eo.id = order_id AND (eo.user_id = (select auth.uid()) OR (select public.is_staff((select auth.uid()))))));

CREATE TRIGGER trading_accounts_updated BEFORE UPDATE ON public.trading_accounts FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER execution_orders_updated BEFORE UPDATE ON public.execution_orders FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
